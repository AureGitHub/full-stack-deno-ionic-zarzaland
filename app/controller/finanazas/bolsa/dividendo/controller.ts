...
const entity = new aureDB(client, clientNoTransaction, entities, 'finanzas.dividendo');
const genericDB = new GenericDB(entity);

const get = async (ctx: any) => {

  const sqlSelect = `  select *   `;
  let sqlFrom = ` from  finanzas.dividendo d     
  `;
...
const update = async (ctx: any) => {
  ...
  try {
    ...
    const carteraEntity = new aureDB(client, clientNoTransaction, entities, 'finanzas.cartera');
    const OldDividendoEntity = new aureDB(client, clientNoTransaction, entities, 'finanzas.dividendo');

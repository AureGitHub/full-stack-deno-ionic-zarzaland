...
const entity = new aureDB(client, clientNoTransaction, entities, 'finanzas.compra');
const genericDB = new GenericDB(entity);

const get = async (ctx: any) => {

  const sqlSelect = `  select *   `;
  let sqlFrom = ` from  finanzas.compra c     
  `;
...
const add = async (ctx: any) => {
  ...
  const transaction = client.createTransaction("transaction_add_compra");
  try {
    const newItem = ctx.state.data;
    const carteraEntity = new aureDB(transaction, clientNoTransaction, entities, 'finanzas.cartera');
    await transaction.begin();
    const cartera = await carteraEntity.findFirst({ where: { id: Number(newItem['carteraid']), tr: transaction } });
...
const update = async (ctx: any) => {
  ...
  const transaction = client.createTransaction("transaction_update_compra");
  try {
    ...
    const OldCompraEntity = new aureDB(client, clientNoTransaction, entities, 'finanzas.compra');
    ...
    if(itemUpdateInput.acciones){
      const acciones =  Number(cartera.acciones) - Number(oldCompra.acciones) +  Number(itemUpdateInput.acciones);

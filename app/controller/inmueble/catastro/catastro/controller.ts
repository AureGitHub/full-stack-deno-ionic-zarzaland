...
const entity = new aureDB(client, clientNoTransaction, entities, 'inmueble.catastro');  
const genericDB = new GenericDB(entity);

const get = async (ctx: any) => {

  const sqlSelect = ` select c.*, descripcion
  `;
  let sqlFrom = ` from  inmueble.catastro c
                  inner join inmueble.catastro_tipo tc on tc.id=c.catastrotipoid
  `;

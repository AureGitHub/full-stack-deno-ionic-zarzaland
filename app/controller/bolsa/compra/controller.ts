...
const entity = new aureDB(client, clientNoTransaction, entities, 'bolsa.compra');
const genericDB = new GenericDB(entity);

const get = async (ctx: any) => {

  const sqlSelect = `  select *   `;
  let sqlFrom = ` from  bolsa.compra c     
  `;
...
const add = async (ctx: any) => {
  ...
  try {
    ...
    const carteraEntity = new aureDB(transaction, clientNoTransaction, entities, 'bolsa.cartera');

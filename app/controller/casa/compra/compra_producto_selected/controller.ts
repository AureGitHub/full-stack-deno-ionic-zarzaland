...
const entity = new aureDB(client, clientNoTransaction, entities, 'casa.compra_producto_selected');
const genericDB = new GenericDB(entity);

const get = async (ctx: any) => {

  const sqlSelect = `  select p.*   `;
  let sqlFrom = ` from  casa.compra_producto_selected  ps
                  inner join casa.compra_producto p on p.id=ps.id
  `;
